...
import { parseTutorRequest, tutorLimits, type TutorMessage } from "../../../lib/tutor-contract";
...
export async function POST(request: Request) {
  ...
  if (!apiKey || !model) {
    return NextResponse.json(
      { error: "The AI tutor is not configured yet. The authored lesson remains fully available." },
      { status: 503 }
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: "The tutor could not be reached. Your pending question can be retried when the connection returns." },
      { status: 503 }
    );
  }
}
