...
export function LessonPanel({
  ...
  async function importMachineEvidence(file: File | undefined) {
    ...
    try {
      ...
      if (!artifact.attestation || !artifact.envelope) {
        setExerciseRecorded(false);
        setMachineVerificationMessage(
          "Unsigned machine evidence is not accepted. Import the signed challenge result generated by the SSH provider."
        );
        return;
      }

      const authoritativeEvidence =
        await submitVerificationAttestationAction(artifact.attestation);

      const isFullExerciseVerification = runtimeTask.scope === "exercise";
      setExerciseRecorded(isFullExerciseVerification);
      setMachineResults(artifact.envelope.stepResults ?? []);
      setMachineVerificationMessage(
        isFullExerciseVerification
          ? "Signed SSH execution was verified by the learning server."
          : "Signed SSH probe evidence was verified by the learning server. Complete the required hands-on exercise to unlock the lesson."
      );
      onEvidenceRecorded?.(authoritativeEvidence);
    } catch (error) {
      setExerciseRecorded(false);
