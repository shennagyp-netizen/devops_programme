import{useState}from"react";import type{Lesson}from"../data/curriculum";import{MotionIllustration}from"./MotionIllustration";
...
export function LessonPanel({lesson,mastered,onMaster}:{lesson:Lesson;mastered:boolean;onMaster:()=>void}){
 ...
 <MotionIllustration lesson={lesson}/><nav className="mode-tabs">{(["learn","listen","do","recall","design"]as Mode[]).map(x=><button key={x} className={m===x?"active":""} onClick={()=>setM(x)}>{x}</button>)}</nav>
 ...
 {m==="listen"&&<div className="content-card"><h3>Podcast</h3><p>Use the daily text file with Google AI Studio and the podcast prompt.</p><pre><code>{lesson.podcast}</code></pre></div>}
