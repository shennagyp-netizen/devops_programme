 ...
 return <div className="podcast-coach">
  ...
  {phase==="brief"&&<div className="coach-grid">
    <div className="content-card">
      ...
      <textarea value={answer} onChange={e=>setAnswer(e.target.value)} placeholder="Write your prediction in your own words." />
      <button className="primary" onClick={()=>setPhase("listen")} disabled={!answer.trim()}>Start guided listening</button>
    </div>
    <div className="content-card coach-rule">
      <h4>How to use the audio</h4>
      <p>Play the episode in your normal audio player. Keep this panel open. When the coach says “stop and predict,” pause the audio here and answer before continuing.</p>
      <p>Do not read the transcript instead of listening on the first pass. The transcript is a recovery path when a sentence was hard to catch.</p>
    </div>
  </div>}
  ...
  {phase==="coach"&&<div className="coach-grid">
    <div className="content-card">
      ...
      <p>{lesson.recall[Math.min(1,lesson.recall.length-1)]}</p>
      <textarea value={answer} onChange={e=>setAnswer(e.target.value)} placeholder="Explain it like you are talking to another engineer." />
  ...
  {phase==="recall"&&<div className="content-card">
    ...
    {lesson.recall.map((q,i)=><label className="recall-row" key={q}><span>{i+1}. {q}</span><textarea value={recallDone[i]?answer:""} onChange={e=>setAnswer(e.target.value)} placeholder="Answer out loud, then tick complete." /><button onClick={()=>setRecallDone(r=>r.map((v,j)=>j===i?!v:v))}>{recallDone[i]?"Done ✓":"Complete"}</button></label>)}
  ...
  {phase==="done"&&<div className="content-card success-card">
    ...
    <button className="primary" onClick={()=>{setPhase("brief");setTurnIndex(0);setAnswer("");setRecallDone(lesson.recall.map(()=>false));}}>Replay as a second pass</button>
  </div>}
 </div>
}
